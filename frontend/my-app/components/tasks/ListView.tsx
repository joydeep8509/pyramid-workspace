...
export function ListView({ tasks, visibleFields, onEdit, onDelete }: ListViewProps) {   
  ...
  const TableHeader = () => (     
    <div className="hidden md:grid grid-cols-12 gap-4 px-6 py-3 border-b border-[#e4e4e7] dark:border-[#27272a] text-[12px] font-semibold text-muted-foreground bg-[#fafafa] dark:bg-[#18181b]/50 rounded-t-[12px]">       
      ...
      <div className="mb-6">         
        ...
        <div className="border border-[#e4e4e7] dark:border-[#27272a] rounded-[12px] bg-white dark:bg-[#09090b] overflow-visible shadow-[0_1px_2px_0_rgba(0,0,0,0.02)]">           
          ...
          {sectionTasks.map((row) => (             
            <div key={row.id} className="relative flex flex-col md:grid md:grid-cols-12 gap-3 md:gap-4 px-4 md:px-6 py-4 md:py-3.5 border-b border-[#e4e4e7] dark:border-[#27272a] last:border-0 md:items-center hover:bg-[#fafafa] dark:hover:bg-[#18181b]/50 transition-colors">               
              <div className="col-span-12 md:col-span-6 text-[14px] md:text-[13px] font-medium text-foreground pr-8 md:pr-0">{row.title}</div>                              
              
              {visibleFields.priority && (                 
                <div className={`col-span-12 md:col-span-2 flex items-center gap-2 text-[13px] font-medium ${getPriorityColor(row.priority)}`}>                   
              ...
              {visibleFields.members && (                 
                <div className="col-span-12 md:col-span-2 flex items-center mt-1 md:mt-0">                   
              ...
              <div className="absolute top-3 right-3 md:relative md:top-0 md:right-0 md:col-span-2 flex justify-end text-muted-foreground action-menu-container">                 
          ...
          <div className="px-4 md:px-6 py-3.5 flex items-center gap-2 text-[13px] font-medium text-muted-foreground hover:bg-[#fafafa] dark:hover:bg-[#18181b]/50 cursor-pointer rounded-b-[12px] transition-colors border-t border-[#e4e4e7] dark:border-[#27272a]">             
